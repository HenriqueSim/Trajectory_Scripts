...
# CORRECT NED mapping: b = g*e3 - a
# therefore:
# bx = -ax
# by = -ay
# bz = g - az
# nonlinear attitude equalities (NED): b = u * R e3  (yaw=0)
# Using b = g*e3 - a, match components and form equalities
# ax + u*sin(th)*cos(ph) = 0
# ay - u*sin(ph) = 0
# g - az - u*cos(ph)*cos(th) = 0
eq1 = ax + u_sym*ca.sin(th_sym)*ca.cos(ph_sym)
eq2 = ay - u_sym*ca.sin(ph_sym)
eq3 = ca.DM(grav) - az - u_sym*ca.cos(ph_sym)*ca.cos(th_sym)
...
# Remember: b = g e3 - a  => b_dot = - a_dot = -jerk
...
g_cons.extend([
    A11*ud_sym + A12*thd_sym + A13*phd_sym - jx,
    A21*ud_sym + A22*thd_sym + A23*phd_sym - jy,
    A31*ud_sym + A32*thd_sym + A33*phd_sym - jz
])
lbg.extend([0.0, 0.0, 0.0])
ubg.extend([0.0, 0.0, 0.0])
...
# CORRECT b = g*e3 - a  (NED mapping)
bx_vals = -ax_vals
by_vals = -ay_vals
bz_vals = grav - az_vals

u_vals = np.sqrt(bx_vals**2 + by_vals**2 + bz_vals**2)
eps = 1e-12
# phi extraction consistent with above: ay = u*sin(phi) -> phi = asin( ay / u )
phi_vals = np.arcsin(np.clip(ay_vals / (u_vals + eps), -1.0, 1.0))
# theta from atan2(bx, bz) (bx = -ax, bz = g - az)
theta_vals = np.arctan2(bx_vals, bz_vals)

# compute Euler-rate time series by solving A(t) * rates = b_dot(t) = -jerk(t)
...
for k, tt in enumerate(ts):
    ...
    theta = theta_vals[k]
    # b_dot = -jerk
    j = np.array([-jx_vals[k], -jy_vals[k], -jz_vals[k]], dtype=float)
...
axs[0,1].set_title('Acceleration'); axs[0,1].legend(); axs[0,1].grid(True)
...
axs[1,1].plot(ts, u_vals, label='u (||g*e3 - a||)'); axs[1,1].axhline(grav, color='k', linestyle=':', linewidth=0.8, label='g'); axs[1,1].axvline(t_mid, color='k', linestyle=':', linewidth=0.8)
axs[1,1].set_title('u (acc norm)'); axs[1,1].set_xlabel('Time [s]'); axs[1,1].set_ylabel('m/s^2'); axs[1,1].legend(); axs[1,1].grid(True)
plt.tight_layout()
...
axs2[0].axvline(t_mid, color='k', linestyle=':', linewidth=0.8); axs2[0].set_title('Euler rates (from jerk)'); axs2[0].legend(); axs2[0].grid(True)
...
bx_m = -ax_m; by_m = -ay_m; bz_m = grav - az_m
u_m = np.sqrt(bx_m**2 + by_m**2 + bz_m**2)
phi_m = np.arcsin(np.clip(ay_m / (u_m + 1e-12), -1.0, 1.0))
...
j_mid = np.array([-jx_m, -jy_m, -jz_m], dtype=float)  # note b_dot = -jerk
